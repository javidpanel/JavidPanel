# راهنمای نصب JavidPanel Installer v0.5.3

## نصب روی Windows

1. بسته `JavidPanel-InstallerBot-v0.5.3.zip` را Extract کنید.
...
> در v0.5.3 دیگر `ADMIN_PASSWORD` و `ADMIN_SECRET` به‌عنوان Worker Secret لازم نیستند. فقط `MASTER_KEY` Secret بوت‌استرپ است. رمز مدیر با PBKDF2-SHA256 در `BOT_KV` ذخیره می‌شود.
...
```text
JavidPanel-v0.5.3-wrapper.obf.js
```

4. در Admin → «نسخه‌ها» فقط همین یک فایل را Upload کنید و تأیید کنید که فایل به‌صورت دستی مبهم شده است.
5. Installer شماره نسخه را از نام فایل تشخیص می‌دهد (فیلد Version فقط override اختیاری است)، فایل کامل را عیناً ثبت و SHA-256ها را محاسبه می‌کند.
